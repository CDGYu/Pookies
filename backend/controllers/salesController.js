...
function createSale(req, res) {
  ...
  const tx = db.transaction(() => {
    const info = insertSale.run({
      ...
      amount_tendered: payment.amountTendered != null ? p4(payment.amountTendered) : null,
      change_due: payment.changeDue != null ? p4(payment.changeDue) : null,
...
function rowToSale(db, row) {
  ...
  return {
    ...
    payment: {
      ...
      amountTendered: row.amount_tendered != null ? p4(row.amount_tendered) : null,
      changeDue: row.change_due != null ? p4(row.change_due) : null,
